> **Implementation status note (added on `restore-work`, branched from
> `restore/pre-cloudflare-2501bd2`):** This document was authored on an
> earlier branch (`main`, checkpoint `7070c45`) alongside a fully-coded
> companion/memory/pattern/chapter/event system. That code was intentionally
> **not** carried into this branch — it was built too early, before the
> 8-question core flow had been shown to real users. This file is kept as
> **vision only**. Nothing described below (memories beyond the 8-question
> journey, pattern discovery, companion chat) exists in this branch's code
> yet. Do not treat this document as a sprint backlog — it is the long-term
> "why," not the next task. Before building any of it, see
> `docs/MANAGEMENT/ROADMAP.md` and confirm the 8-question → results flow has
> real user validation first.
...
Sound turns that lived relationship with music into something visible,
