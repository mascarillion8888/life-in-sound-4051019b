...
- **Vitest:** 70 dosya, 691 passed / 2 skipped (0 failed) — `npm test` exit 0 (doğrulandı 2026-09-24). visualSpec 19/19.
...
1. **Mood-backdrop doğrulaması — ✅ TAMAMLANDI (kullanıcı onayı, 2026-09-20):** `npm run dev` → journey 8 şarkıyla tamamlandı → her EraCardReveal'da şarkının mood'uyla eşleşen wallpaper görüldü, kullanıcı "Arayüz Onaylandı" dedi. ⚠️ Mood inference gerçek OpenRouter key ister (`.env`'de `OPENROUTER_API_KEY`).
   ...
   **DÜZELTİLDİ (2026-09-24, yaş aralığı çakışması) — `ageRanges` dizisi:** BUG (2026-09-24'te açıldı): `lifeCards.ts`'te `ageRanges[4]="Ages 18-28"` komşularıyla (index 3="Ages
   18-22", index 5="Ages 23-30") üst üste biniyordu — hem İngilizce hem Türkçe kopyada; `data.ts` eras[].age'de de aynıydı. **FIX:** kesişmeyen, ardışık genç-yetişkin bandı — `[4]="Ages 23-29"`, `[5]="Ages 30-39"`, `[6]="Ages 40+"` (lifeCards EN+TR) + `data.ts` id5/6/7 age aynen. Tutarlılık için data.ts de dahil edildi (progress-bar ≠ kart etiketi sapmasını önler). KURAL-10: yalnız görünür etiket metni (QuizCard/MasterPoster/poeticPoster yüzeyinde), layout/asset değişmedi → küçük içerik düzeltmesi, tam göz-onayı gerektirmez.
...
13. **Tanı-yasağı + anti-klişe kuralları gerçek LLM promptlarına (2026-09-24, 55954b2) ✅ TAMAMLANDI:** 4 prose prompt'a eklendi — `src/lib/llm/prompts.ts` `GROUNDING_RULES` (life-story: "never a clinician... no mental-health conditions... maps of feeling are reflections, not diagnoses" + "never as medical evidence" + "no horoscope-generic, fortune-cookie..." ), `src/lib/llm/poetic-analyzer.ts` `ANALYZER_GROUNDING_RULES` (aynen 3 satır), `src/lib/llm/generateAnalysis.server.ts` `buildEntryInsightPrompt` kural 5-6 ("never a clinician", "avoid generic fortune-cookie"), `src/lib/art/generateCard.server.ts` `buildLorePrompt` ("not a clinical case", "never a generic fortune-cookie compliment"). `src/lib/ai/moodInference.server.ts` sistem satırı → "never a clinician or diagnostician; you only label a song's mood". **Davranış kanıtı (gerçek OpenRouter, temp 0.8):** aynı 8-şarkılık journey'nin OLD vs NEW life-story çıktısı karşılaştırıldı — NEW'da "testament to... define your spirit" türü jenerik/fortune-cookie satırları belirgin azaldı, anlatı şarkı gerçekliğine demirlendi; regresyon yok. Testler: 5 dosyada prompt-satırı `toContain` (+7, tam suite 691/2). Kural-10: hayır (prompt içeriği). `buildLorePrompt` test edilebilirlik için export edildi.
14. **Bulgu 2 — era-badge/portal yaşları düzeltildi (2026-09-24, bu commit) ✅:** `parsePoeticAnalysis` LLM chapter'larında `ageRange` boş/eksikken deterministik fallback'ten (aynı id, yoksa pozisyon) ageRange'i düşüyordu — LLM prompt şeması `ageRange` taşımadığı için (buildPoeticAnalyzerPrompt TASK) her LLM chapter'ı boş yaşla render oluyordu (MasterPosterSheet era-badges + portal-strip boş metin). Çözüm: (ii) karar — LLM şemasına dokunmadan, parse'ta `asNonEmptyString(chapter.ageRange) ?? fallbackAgeRange` (id eşleşmesi → pozisyon). 2 test eklendi (eksik ageRange → fallback'ten dolar; sağlanmış ageRange → korunur). Suite 693/2, tsc 0. Kural-10: hayır (veri düzeltmesi, görsel layout aynı).
15. **Bulgu 1 — Cinematic Poster canvas: LOCALDE DOĞRULANDI ÇALIŞIYOR, production bildirimi açık (2026-09-24) 🔵:** Hermes local repro'da (`npm run dev` + gerçek 8-şarkılık journey) `getImageData` ile kanıtladı: canvas `nonZeroRatio=1.0` (2.8M px %100 boyalı), 1400×2000, 27.315 açık metin pikseli — `renderPoster` deterministik çizim çalışıyor, "boş" değil. "Placeholder cinematic poster" aria-label'i yalnız `posterAlt` i18n string'i (dictionaries.ts:200), çizim-yok kanıtı değil. **Ancak:** kullanıcı Vercel PRODUCTION'da kalıcı boş gördüğünü bildirmişti (local'de değil). Bu, local dev vs production build arası hydration-TIMING farkı olabilir (Suspense/lazy-load/CDN-cache), henüz production'da repro EDİLMEDİ. Hermes canlı production /results'ta yeniden ölçüm yapacak; tekrar boş görürse **production'da (local'de değil) gerçek DevTools ölçümü** yapılmalı. NOT: Canvas hydration-öncesi SSR anında 0×0 boştur (GeneratedPoster.tsx:14 yorumu bunu zaten söyler). ayrıca: `posterAlt` "Placeholder" metni yanıltıcı — Sonraki temizlik turunda düzeltilecekler listesine eklendi (bu turda dokunulmadı).

---

## 6. Sıradaki İş Adımları (Next Steps)

0. **MUSIC DNA P0 (büyük mimari iş — ayrı oturum, sıradaki büyük hedef):** gerçek şarkı verisi üzerinden analitik çekirdek. Şu an ürün büyük ölçüde soru-ağırlıklı: grounded pipeline şarkıları besliyor (results.tsx:411 `generateGroundedAnalysis(songs)`; genre iTunes'dan, mood inference per-song) ama Music DNA'nın AĞIRLIKLI çekirdeği (personality scoring / dominantVibe 2-sabit-etiket) hâlâ `answers` üzerinden; şarkı metadata'sı henüz Music DNA'nın temel girdisi değil. Detay: Gap Analysis §8-§10, ANA_YASA §1/§3/§8 (P0). Bu, yeni özellik eklemek değil; mevcut MVP'nin analitik kalbini düzeltmek.
...
```
55954b2 feat(ai): tanı-yasağı + anti-cliché rules into 4 prose prompts + mood-inference non-clinician system line + tests
9fff09c docs(handoff): sync §1 HEAD→b44f945 + CrisisGuard/ETHICAL_AI record; PROJECT_STATUS: Option C in §3, kriz-güvenliği in §4
